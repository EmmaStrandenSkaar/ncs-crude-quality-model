...
RESULTS = Path("/Users/emmastrandenskaar/Documents/Claude/Projects/Oljepris/analyses/decline_quality/results")
DOCS = Path("/Users/emmastrandenskaar/Documents/Claude/Projects/Oljepris/analyses/decline_quality/docs")
OUTPUT = DOCS / "NCS_Decline_Model_Methodology.docx"  # overwrite v2
ONEDRIVE = Path("/Users/emmastrandenskaar/Library/CloudStorage/OneDrive-BINorwegianBusinessSchool(BIEDU)/NCS_Decline_Model_Methodology.docx")
...
add_para("Fysikk + Premium Rammeverk for Equity Research",
         ...
         space_after=4)
add_para("Versjon 2.1 — med QA-fikser & nested CV",
...
add_table_data_row(stats_table, ["Nested CV R²", "0.662"], bold=[False, True], colors=[None, GREEN])
add_table_data_row(stats_table, ["In-sample R²", "0.742"])
add_table_data_row(stats_table, ["RMSE", "0.042"])
add_table_data_row(stats_table, ["NCS-felt med reservoar-API", "31 av 48"], bold=[False, True], colors=[None, GREEN])
add_table_data_row(stats_table, ["Felt med felt-spesifikk reservoartemperatur", "15"])
add_table_data_row(stats_table, ["Aker BP MAE", "0.040"])
add_table_data_row(stats_table, ["Aker BP treffrate (±0.05)", "83% [55-95% CI]"], bold=[False, True], colors=[None, GREEN])
...
add_para("Modellen oppnår ærlig nested CV R² = 0.662 og Aker BP treffrate på 83% (innen ±0.05). Alle inputs er offentlig tilgjengelige fra Sodir og operatør-rapporter.")
